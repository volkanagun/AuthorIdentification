...
object MLUtilsModified {
  ...
  /**
    ...
    *
    * @see [[org.apache.spark.mllib.util.MLUtilsModified#loadLibSVMFile]]
    */
  @Since("1.0.0")
  ...
  /**
    ...
    *             [[org.apache.spark.mllib.util.MLUtilsModified#loadLabeledPoints]] for loading.
    */
  @Since("1.0.0")
  ...
  @Since("1.0.0")
  @Experimental
  def kFoldEqually[T: ClassTag](rdd: RDD[T], numFolds: Int, seed: Int): Array[(RDD[T], RDD[T])] = {
    val numFoldsF = numFolds.toFloat
    val sc = rdd.sparkContext
    val myrdd = rdd.groupBy(row=> row.asInstanceOf[Row].getString(0))
    val largeSequence: Array[(String, Iterable[T])] = myrdd.collect()
    ...
    (1 to numFolds).map { fold =>
      ...
      var trainingRDD: RDD[T] = sc.emptyRDD[T]
      var validationRDD: RDD[T] = sc.emptyRDD[T]

      largeSequence.foreach(pair => {

        val current: RDD[T] = rdd.sparkContext.makeRDD(pair._2.toList)
        val validation = new PartitionwiseSampledRDD(current, sampler, true, seed)
        val training = new PartitionwiseSampledRDD(current, sampler.cloneComplement(), true, seed)

        trainingRDD = trainingRDD.union(training)
        validationRDD = validationRDD.union(validation)

      })

      (trainingRDD, validationRDD)
    }.toArray.tail
  }
  ...
  /**
    * Spark Context creation for tests here
    * @return
    */
  def context(): SparkContext = {
    val conf = new SparkConf()
      .setAppName("mlutils-tests")
      .setMaster("local[8]")
  ...
  def main(args: Array[String]) {
    val sc = context();
    val list = List((1d, "1.A"), (1d, "1.B"), (1d, "1.C"), (1d, "1.D"), (1d, "1.E"), (1d, "1.F"), (2d, "2.A"), (2d, "2.B"), (2d, "2.C"), (2d, "2.D"), (2d, "2.E"), (2d, "2.F"))
    val rddNot = sc.makeRDD(list)
    val rdd: RDD[(Double, Iterable[(Double, String)])] = rddNot.groupBy(_._1)
    val arrayEqually = kFoldEqually(rdd, 4, 0)
    val arrayNot = kFold(rddNot, 2, 0)
    val buffer = new StringBuilder("")

    //rdd.collect().foreach(pair=>buffer.append(pair._1+"=>"+pair._2.toList.toString()+"\n"))

    arrayEqually.foreach(pair => {
      buffer.append("Training:\n")
      pair._1.collect().foreach(item => {
        buffer.append("->" + item._2)
      })
      buffer.append("\n")
      buffer.append("Testing:\n")
      pair._2.collect().foreach(item => buffer.append("->" + item._2))
      buffer.append("\n")
    })

    buffer.append("++++++++++++++++++++++++++++++++++++++++++++++++++++\n")

    arrayNot.foreach(pair => {
      buffer.append("Training:\n")
      pair._1.collect().foreach(item => {
        buffer.append("->" + item._2)
      })
