...
/**
  ...
  */
@Experimental
class MultilayerPerceptronClassifier(override val uid: String) extends Predictor[Vector, MultilayerPerceptronClassifier, MultilayerPerceptronClassificationModel]
  ...
  /**
    * Output column and label column
    * @param extra
    * @return
    */
  ...
  override protected def train(dataset: DataFrame): MultilayerPerceptronClassificationModel = {
    ...
    val inputSize = dataset.select($(featuresCol)).first().getAs[Vector](0).size
    val myLayers = inputSize+:$(layers)
