...
package org.apache.spark.ml.bases
...
/**
  ...
  */
private[ml] trait MultiPredictorParams extends Params
  ...
  protected def validateAndTransformSchema(
    ...
    if (fitting) {
      ...
      SchemaUtils.checkColumnType(schema, $(labelCol), featuresDataType,"Multi Predictor expects multiple double type outputs")
    }
    SchemaUtils.appendColumn(schema, $(predictionCol), featuresDataType)
  }
}
...
/**
  ...
  * @tparam M            Specialization of [[MultiPredictionModel]].  If you subclass this type, use this type
  *                      parameter to specify the concrete type for the corresponding model.
  */
@DeveloperApi
abstract class MultiPredictor[
FeaturesType,
Learner <: MultiPredictor[FeaturesType, Learner, M],
M <: MultiPredictionModel[FeaturesType, M]]
  extends Estimator[M] with MultiPredictorParams {
  ...
  /**
    ...
    */
  protected def extractLabeledPoints(dataset: DataFrame): RDD[MultiLabeledPoint] = {
    dataset.select($(labelCol), $(featuresCol)).rdd
      .map { case Row(label: Vector, features: Vector) => MultiLabeledPoint(label, features) }
  }
}
...
/**
  ...
  */
@DeveloperApi
abstract class MultiPredictionModel[FeaturesType, M <: MultiPredictionModel[FeaturesType, M]]
  extends Model[M] with MultiPredictorParams with DefaultParamsWritable {
  ...
  /**
    ...
    */
  protected def predict(features: FeaturesType): Vector
}
