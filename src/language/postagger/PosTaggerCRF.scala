...
import classification.postagging.{CaseLabelSet, PennLabelSet, SelectedTagSet}
import data.document.{Context, Sentence, Window}
import language.morphology.{EmptyTagModel, HasimAnalyzerImp, MorphLight, AnalyzerImp}
...
/**
  ...
  */
class PosTaggerCRF() extends Serializable {
  //Mallet trainer
  val context = new Window[String](3)
  val pipe = new PosTaggerPipe(context)
  val modelFilename = Resources.MorphologyResources.crfMorphoTag
  ...
  def test(tokens: Seq[String]): Seq[(String, String)] = {
    test(classifier, tokens)
  }

  def test(tagger:CRF, tokens: Seq[String]): Seq[(String, String)] = {
    val myInstance = instance(tokens)

    val edInstance = tagger.transduce(myInstance)
...
object PosTaggerCRF {
  ...
  val trainData = Resources.MorphologyResources.loadMorphology(testFilename)
  val testData = Resources.MorphologyResources.loadMorphology(testFilename)

  val posTaggerCRF = new PosTaggerCRF()

  def main(args: Array[String]) {
    //val classifier = posTaggerCRF.train(trainData,24)
    val annotation = posTaggerCRF.test(Seq("Ali","kendini", "gösterdi",".")).map(pair=>pair._1+"/"+pair._2).mkString(" ")
    println(annotation)
    //posTaggerCRF.evaluate(trainData, testData, 24)
  }
}

class PosTaggerPipe(val context: Window[String]) extends Pipe(new Alphabet(), new LabelAlphabet()) {
  ...
  protected def extract(instance: Instance, targetProcessing: Boolean = false): Instance = {
    ...
    val tokens = tokenLabels.map(tokenLabel => (tokenLabel._1, SelectedTagSet.enrichTags(tokenLabel._2)))
